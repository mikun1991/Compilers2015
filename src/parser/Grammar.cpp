...
bool Grammar::programHeading()
{
	...
	case MP_PROGRAM:
		...
		match();
<<<<<<< HEAD
		_semanticAnalyser.createTable(nextToken(), VoidData);
		programIdentifier();
=======
...
SemanticRecord Grammar::programIdentifier()
{
...
{
	SemanticRecord valueParamSection_rec;

	switch (nextTokenType())
	{
	case MP_IDENTIFIER:
		logRule(27);
		identifierList(valueParamSection_rec);
		if (nextTokenType() == MP_COLON)
		{
			match();
			type(valueParamSection_rec);
			return true;
		}
		else {
			error(TypeList() << MP_COLON);
...
{
	SemanticRecord variableParamSection_rec;
	...
	case MP_VAR:
		logRule(28);
		match();
		identifierList(variableParamSection_rec);
		if (nextTokenType() != MP_COLON){
			error(TypeList() << MP_COLON);
			return false;
		}
